...
def grid_show(to_shows, cols, num):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            axs[i, j].set_yticks([])
            axs[i, j].set_xticks([])
    plt.savefig('./result/attention_visual/head_grid/savefig_{}.jpg'.format(num))
    plt.show()
...
def visualize_heads(att_map, cols, num):
    to_shows = []
    ...
    grid_show(to_shows, cols=cols, num=num)
...
def visualize_grid_to_grid_with_cls(att_map, grid_index, image, grid_size=14, alpha=0.6, num=None, head=None):
    if not isinstance(grid_size, tuple):
        grid_size = (grid_size, grid_size)
    ...
    ax[1].axis('off')
    plt.savefig('./result/attention_visual/attn_cls_grid/savefig_{}_{}.jpg'.format(head,num))
    plt.show()


def visualize_grid_to_grid(att_map, grid_index, image, grid_size=14, alpha=0.6, num=None, head=None):
    if not isinstance(grid_size, tuple):
        grid_size = (grid_size, grid_size)
    image = Image.fromarray(image)
    H,W = att_map.shape
    ...
    ax[1].axis('off')
    plt.savefig('./result/attention_visual/attn_grid/savefig_{}_{}.jpg'.format(head,num))
    plt.show()
